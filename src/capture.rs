...
impl<'a> Event<'a> {
    /// Decodes the tagged event union into the matching variant.
    ///
    /// # Safety
    ///
    /// `event` must be delivered by lore, with `tag` selecting the union
    /// field the producer initialized.
        ...
        unsafe {
            match tag {
                LORE_EVENT_LOG => Self::Log {
                    ...
                    message: data.log.message.as_str(),
                },
                LORE_EVENT_ERROR => Self::Error {
                    error_type: data.error.error_type,
                    message: data.error.error_inner.as_str(),
                },
                ...
                LORE_EVENT_REPOSITORY_STATE_DUMP_NODE => Self::RepositoryStateDumpNode {
                    path: data.repository_state_dump_node.name.as_str(),
                },
